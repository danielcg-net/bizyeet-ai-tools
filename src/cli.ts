...
type CliRuntime = Readonly<{
  ...
  listQuotes?: (input: Omit<Parameters<typeof listQuotes>[0], "fetcher" | "metadata" | "now">) => Promise<AgentResult>;
...
const runtime: CliRuntime = {
  ...
  listQuotes: async (input) => listQuotes({ ...input, fetcher: fetch, now: Date.now, metadata: () => discoverOAuth(new URL(input.profile.issuer), fetch) }),
...
const helpMessage = [
  ...
  "       bizyeet catalog get <opaque-id> [--fields <name,...>] [--profile <name>] [--export]",
...
const crmListOptions = (resource: "customers" | "leads" | "payments" | "services" | "quotes" | "catalog", args: readonly string[]): PaymentListOptions => {
...
const crmRead = async (resource: "customers" | "leads" | "payments" | "services" | "quotes" | "catalog", args: readonly string[], dependencies: CliStorage, execution: CliRuntime): Promise<CliResult> => {
  ...
  try {
    ...
    if (resource === "services" && (!validServiceReadFields(listOptions?.fields ?? fields, command === "get")
      || (listOptions && (!Number.isInteger(listOptions.limit) || (listOptions.limit ?? 25) < 1 || (listOptions.limit ?? 25) > 100)))) return invalidInput("Service read options are invalid.");
    ...
    const list = resource === "catalog" ? execution.listCatalog : resource === "quotes" ? execution.listQuotes : resource === "services" ? execution.listServices : resource === "customers" ? execution.listCustomers : resource === "leads" ? execution.listLeads : execution.listPayments;
    const get = resource === "catalog" ? execution.getCatalogItem : resource === "quotes" ? execution.getQuote : resource === "services" ? execution.getService : resource === "customers" ? execution.getCustomer : resource === "leads" ? execution.getLead : execution.getPayment;
...
const recordUpdate = async (resource: "customers" | "leads", args: readonly string[], dependencies: CliStorage, execution: CliRuntime): Promise<CliResult> => {
  ...
  if (mode !== "preview" && mode !== "execute" && mode !== "status") return invalidInput(`Use ${resource} update preview, execute, or status.`);
  const preview = resource === "customers" ? execution.previewCustomerUpdate : execution.previewLeadUpdate;
  const executeUpdate = resource === "customers" ? execution.executeCustomerUpdate : execution.executeLeadUpdate;
  const status = resource === "customers" ? execution.customerUpdateStatus : execution.leadUpdateStatus;
  ...
  try {
    ...
    if (mode === "preview") {
      if (!execution.readChanges || !preview) throw new Error("Write runtime unavailable");
      return resourceOutput(await preview({ ...session, proposal: { resource_id: id, changes: await execution.readChanges() } }));
    }
    if (mode === "status") {
      if (!status) throw new Error("Status runtime unavailable");
      return resourceOutput(await status({ ...session, query: { preview_id: id, idempotency_key: key } }));
    }
...
export const run = async (args: readonly string[], dependencies: CliStorage = storage, execution: CliRuntime = runtime, onVerification: (device: DeviceAuthorization) => void = () => undefined): Promise<CliResult> => {
  ...
  if (dependencies.withProfileLock && (first === "catalog" || first === "quotes" || first === "services" || first === "customers" || first === "leads" || first === "payments" || first === "expenses" || first === "reports" || first === "bookings" || first === "auth")) {
    try {
