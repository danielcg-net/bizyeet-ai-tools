...
export const paymentSummaryResponse = (value: unknown, requested: PaymentSummaryOptions): Readonly<Record<string, unknown>> | undefined => {
  ...
  if (!instant(period.start) || !instant(period.end) || period.start >= period.end || data.start !== period.start || data.end !== period.end
    || typeof period.range !== "string" || !(paymentSummaryRanges as readonly string[]).includes(period.range) || period.range !== (requested.range ?? "month")
    || period.requestedStartDate !== (requested.start_date ?? null) || period.requestedEndDate !== (requested.end_date ?? null)
  ...
  return Object.freeze({ data: Object.freeze({
    ...
    period: Object.freeze({ range: period.range, timeZone: period.timeZone, start: period.start, end: period.end,
      requestedStartDate: period.requestedStartDate, requestedEndDate: period.requestedEndDate }),
