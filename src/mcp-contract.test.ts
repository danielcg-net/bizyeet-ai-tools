...
void test("publishes only bounded, read-only MCP tools", () => {
  assert.deepEqual(mcpReadTools.map((tool) => tool.name), ["bizyeet_customers_list", "bizyeet_customers_get", "bizyeet_leads_list", "bizyeet_leads_get", "bizyeet_payments_list", "bizyeet_payments_get", "bizyeet_payments_list_with_relationships", "bizyeet_payments_get_with_relationships", "bizyeet_payments_received_summary", "bizyeet_reports_taxes", "bizyeet_expenses_list", "bizyeet_expenses_get", "bizyeet_expense_schedules_list", "bizyeet_expense_schedules_get"]);
  ...
  assert.ok(mcpReadTools.every((tool) => tool.securitySchemes[0]?.scopes[0] === (tool.name.startsWith("bizyeet_payments_") ? "payments.read" : tool.name.startsWith("bizyeet_expense") ? "expenses.read" : tool.name === "bizyeet_reports_taxes" ? "reports.read" : "customers.read")));
