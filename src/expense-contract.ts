...
export const expenseReadFields = Object.freeze(["id", "schedule_id", "period_start", "name", "category", "vendor", "amount", "currency", "incurred_on", "paid_on", "status", "notes", "created_at", "updated_at"] as const);
export const expenseSortFields = Object.freeze(["name", "category", "vendor", "amount", "currency", "incurred_on", "paid_on", "status", "created_at", "updated_at"] as const);
...
export const validExpenseListOptions = (value: unknown): value is ExpenseListOptions => {
  ...
  if (Object.keys(input).some((key) => !["fields", "page_size", "cursor", "search", "sort", "dir", "status", "category", "currency", "schedule", "start_date", "end_date"].includes(key))) return false;
  return (input.page_size === undefined || (typeof input.page_size === "number" && Number.isInteger(input.page_size) && input.page_size >= 1 && input.page_size <= 100))
    ...
    && (input.schedule === undefined || (typeof input.schedule === "string" && input.schedule.length > 0 && optionalText(input.schedule, 512)))
    && calendarDate(input.start_date) && calendarDate(input.end_date)
    && (input.start_date === undefined || input.end_date === undefined || input.start_date <= input.end_date);
};
