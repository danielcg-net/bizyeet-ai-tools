...
await Promise.all(["customers", "leads"].flatMap((resource) => ["--profile", "--profile=other"].flatMap((id) => ["default", "selected"].map((name) =>
  test(`locks the selected profile for ${resource} separated opaque ID ${id} with ${name}`, async (context) => {
    ...
    const read = async (input: Omit<Parameters<typeof import("./agent-client.js").getCustomer>[0], "fetcher" | "metadata" | "now">): Promise<import("./agent-client.js").AgentResult> => {
      assert.equal(input.resourceId, id);
      assert.deepEqual(input.options, { fields: ["id", "name"] });
      await input.persistCredentials(credentials);
      return { credentials, response: { data: { id }, meta: { contract_version: "v1" } } };
    };
    const result = await run([resource, "get", "--fields", "id,name", ...(name === "default" ? [] : ["--profile", name]), "--", id], {
      ...
      getCustomer: resource === "customers" ? read : forbidden,
      getLead: resource === "leads" ? read : forbidden,
    });
