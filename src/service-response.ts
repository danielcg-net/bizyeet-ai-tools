...
const publicItems = (value: unknown): readonly unknown[] | undefined => {
  if (!Array.isArray(value) || !value.every((line: unknown) => record(line) && validResourceId(line.id)
...
const fieldValue = (field: string, value: unknown): boolean => {
  ...
  if (field === "pricing_revision") return typeof value === "number" && Number.isSafeInteger(value) && value > 0;
  if (field === "duration_minutes") return value === null || (typeof value === "number" && Number.isSafeInteger(value) && value >= 0);
  return value === null || typeof value === "string";
};

/** Project canonical service facts, never provider data or hidden line-item costs. */
export const serviceResponse = (body: unknown, options: Readonly<{ fields?: readonly string[]; page_size?: number }>, id: string | null): Readonly<Record<string, unknown>> | undefined => {
  ...
  const row = (value: unknown): Readonly<Record<string, unknown>> | undefined => {
    ...
    const fields = Object.freeze(["id", ...(options.fields?.length ? options.fields : serviceReadFields.filter((field) => Object.hasOwn(value, field)))
      .filter((field) => field !== "id")]);
    if (!fields.every((field) => serviceReadFields.some((known) => known === field) && Object.hasOwn(value, field)
      && fieldValue(field, value[field]) && (id !== null || field !== "items"))) return undefined;
