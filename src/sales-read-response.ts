...
const fieldValue = (field: string, value: unknown): boolean => {
  ...
  if (field === "pricing_revision" || field === "sent_count") return typeof value === "number" && Number.isSafeInteger(value) && value >= (field === "pricing_revision" ? 1 : 0);
  if (field === "duration_minutes") return value === null || (typeof value === "number" && Number.isSafeInteger(value) && value >= 0);
  return value === null || typeof value === "string";
};

/** Build a public sales projection without interpreting routing or pricing rules. */
export const createSalesResponse = (allowedFields: readonly string[], allowListItems: boolean) => (body: unknown, options: Readonly<{ fields?: readonly string[]; page_size?: number }>, id: string | null): Readonly<Record<string, unknown>> | undefined => {
  ...
  const row = (value: unknown): Readonly<Record<string, unknown>> | undefined => {
    ...
    const fields = Object.freeze(["id", ...(options.fields?.length ? options.fields : allowedFields.filter((field) => Object.hasOwn(value, field)))
      .filter((field) => field !== "id")]);
    if (!fields.every((field) => allowedFields.some((known) => known === field) && Object.hasOwn(value, field)
      && fieldValue(field, value[field]) && (allowListItems || id !== null || field !== "items"))) return undefined;
