...
export const taxReportResponse = (value: unknown, requested: TaxReportOptions): Readonly<Record<string, unknown>> | undefined => {
  ...
  const itemCurrencies = data.items.map((item: unknown) => record(item) && fieldValue("currency", item.currency)
    && typeof item.currency === "string" ? item.currency : undefined);
  if (itemCurrencies.some((currency) => currency === undefined)) return undefined;
  ...
  const totalCurrencies = new Set(totals.map((group) => group?.currency));
  if (items.some((item) => item === undefined) || totals.some((group) => group === undefined
    ...
    || totalCurrencies.size !== totals.length || !itemCurrencies.every((currency) => totalCurrencies.has(currency))) return undefined;
