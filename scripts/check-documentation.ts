...
const hereDocuments = (state: ShellLineState): readonly HereDocument[] => state.hereStarts.map((start) => {
  const suffix = state.text.slice(start + 2);
  const stripTabs = suffix.startsWith("-");
  const delimiter = parseShell(stripTabs ? suffix.slice(1) : suffix, () => "\0dynamic\0")[0];
  if (typeof delimiter !== "string" || delimiter.includes("\0")) throw new Error("Nonliteral heredoc delimiter.");
  return { delimiter, stripTabs };
});

const endsDocument = (line: string, document: HereDocument): boolean =>
  (document.stripTabs ? line.replace(/^\t+/u, "") : line) === document.delimiter;
...
const shellLines = (source: string, bashExample: boolean): readonly string[] => {
  ...
  const completed = Array.from(source.replace(/\r\n/gu, "\n")).reduce<ShellLineState>((state, character) => {
    ...
    if (document) return character === "\n"
      ? { ...state, documentLine: "", documents: endsDocument(state.documentLine, document) ? state.documents.slice(1) : state.documents }
      : { ...state, documentLine: state.documentLine + character };
    if (state.escaped) return character === "\n" ? { ...state, escaped: false }
      : { ...state, escaped: false, boundary: false, descriptorDigits: 0, lessThanRun: 0, text: `${state.text}\\${character}` };
    if (state.comment) return character === "\n"
      ? { ...initial, text: `${state.text}\0`, documents: hereDocuments(state) } : { ...state, text: state.text + character };
    if (character === "\\" && state.quote !== "single") return { ...state, escaped: true };
    if (character === "'" && state.quote !== "double") return { ...state, boundary: false, descriptorDigits: 0, lessThanRun: 0,
    ...
    const lessThanRun = character === "<" ? state.lessThanRun + 1 : 0;
    return { ...state, comment: character === "#" && state.boundary,
      lessThanRun, hereStarts: lessThanRun === 2 ? [...state.hereStarts, state.text.length - 1]
        : lessThanRun === 3 ? state.hereStarts.slice(0, -1) : state.hereStarts,
...
const operator = (word: ShellWord | undefined): string | undefined => typeof word === "object" && "op" in word ? word.op : undefined;
// The tokenizer splits POSIX <> and << into two operators. Both consume one
// word; heredoc payloads are excluded by shellLines before operand selection.
const shellWords = (line: string): readonly ShellWord[] => {
  const words = parseShell(line, () => "\0dynamic\0");
  return words.flatMap((word, index): readonly ShellWord[] => {
    if (operator(word) === "<" && ["<", ">"].includes(operator(words[index + 1]) ?? "")) return [{ op: "<<<" }];
    if (["<", ">"].includes(operator(word) ?? "") && operator(words[index - 1]) === "<") return [];
    return [word];
  });
};
...
const scriptFindings = (file: string, source: string, scripts: ReadonlySet<string>, consoleExample: boolean, bashExample: boolean): readonly DocumentationFinding[] => {
  ...
  try {
    const prompted = consoleExample && /^[\t ]*\$[\t ]+/mu.test(source);
    const commands = prompted ? source.split(/\r?\n/u).filter((line) => /^[\t ]*\$[\t ]+/u.test(line))
